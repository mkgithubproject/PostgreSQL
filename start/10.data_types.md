...
PostgreSQL **does not execute line-by-line like a programming language** — it executes **statements**, and a statement is considered complete **only when you end it with a semicolon (`;`)**.
...
1. **`CREATE TABLE ...`**
   ...
   * You didn’t put a semicolon.
   * PostgreSQL says: *“Okay, this statement isn’t finished yet, I’ll wait.”*
   * It **does not execute anything yet**.

2. **Blank lines & comments**

   * PostgreSQL ignores comments (`-- ...`) and blank lines.
   * Still waiting for the first statement to end.

3. **`SELECT ...`**

   * PostgreSQL now sees another SQL command while still waiting to close the first one.
   * It tries to merge them → ❌ **syntax error**.

👉 Because of this, **nothing runs successfully**. Neither the table is created, nor the `SELECT` executes.

---

✅ **Important rule:**
PostgreSQL runs **top to bottom**, **but only when it knows a statement is complete** (which usually means it ends with `;`).
